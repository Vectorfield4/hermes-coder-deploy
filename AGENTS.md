...
- `make check-secrets` — preflight: every secret file declared in compose exists and required ones are non-empty (optional: `ftp_*`, `vercel_org_id`; `dense_mem_*` missing/empty only warns — they are generated by `make setup`)
