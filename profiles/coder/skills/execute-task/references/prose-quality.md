...
Loaded by `content-strategist` and `narrative-designer` before generating copy. Used by `execute-task` and `pr-judge` as content quality overlay.
...
- CTAs describe the actual next step ("See a 2-min demo" not "Get Started")

## Scoring Guide (used by pr-judge)
